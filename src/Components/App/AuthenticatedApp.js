...
function AuthenticatedApp(props) {
    ...
    return (
        <div className="App" style={{backgroundColor:"#f2f2f2"}}>
            ...
            {results.map((result)=>{
                return <div style={{padding: "80"}}>{<Draggable  defaultClassName={"shadow"}><div style={{width:"400px"}}><ReactJson displayDataTypes={false} style={{width:"400px",backgroundColor:"white"}} theme={"rjv-default"} src={result} /></div></Draggable>
                }</div>
            })}
            <div>{error && error}</div>
        </div>
